...
class ComicscrawlerPipeline:
    def process_item(self, item, spider):
        ...
        if adapter.get('image_url'):
            ...
            ).get_or_create(image_url=adapter['image_url'], slug=adapter['slug'], rating=adapter['rating'], status=adapter['status'], description=adapter['description'], released=adapter['released'],  author=adapter['author'],  artist=adapter['artist'], defaults={'title': adapter['title']})
